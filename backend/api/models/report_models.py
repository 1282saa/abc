...
REPORT_TEMPLATES = {
    ReportPeriodType.DAILY: ReportTemplate(
        ...
        1. 주요 이슈 (하루 동안 가장 중요한 뉴스)
        2. 언론 반응 (언론사들의 반응과 관점)
        3. 시장 영향 (주가나 시장에 미칠 수 있는 영향)
    ...
    ReportPeriodType.WEEKLY: ReportTemplate(
        ...
        1. 주간 하이라이트 (가장 중요한 뉴스들의 요약)
        2. 사업 동향 (사업 관련 주요 발표나 변화)
        3. 언론 보도 분석 (언론의 시각과 관점 분석)
        4. 향후 전망 (다음 주 예상되는 이슈나 관심사)
    ...
    ReportPeriodType.MONTHLY: ReportTemplate(
        ...
        월간 종합 분석 리포트를 작성해주세요:
        
        1. 월간 요약 (한 달 동안의 핵심 이슈들)
        2. 주요 사업 동향 (신사업, 제품 출시, 파트너십 등)
        3. 재무 관련 뉴스 (실적, 투자, 자금조달 등)
        4. ESG 및 지속가능경영 (환경, 사회, 지배구조 관련)
        5. 시장 평가 (애널리스트 의견, 주가 동향)
        6. 향후 한 달 전망 (예상되는 주요 이벤트와 이슈)
    ...
    ReportPeriodType.QUARTERLY: ReportTemplate(
        ...
        분기별 전략 분석 리포트를 작성해주세요:
        
        1. 분기 핵심 성과 (가장 중요한 성과와 이정표)
        2. 전략적 이니셔티브 (새로운 전략, 사업 방향 변화)
        3. 경쟁 환경 분석 (경쟁사 대비 포지션, 시장 변화)
        4. 재무 성과 분석 (분기 실적, 수익성, 성장성)
        5. 리스크 요인 (내외부 리스크와 대응 방안)
        6. 다음 분기 전략 방향 (예상되는 전략과 중점 사항)
    ...
    ReportPeriodType.YEARLY: ReportTemplate(
        ...
        연간 종합 분석 리포트를 작성해주세요:
        
        1. 연간 성과 요약 (올해의 가장 중요한 성과들)
        2. 사업 포트폴리오 변화 (사업 구조 변화, 신규 진출, 철수)
        3. 글로벌 전략 (해외 사업 확장, 글로벌 파트너십)
        4. 혁신과 R&D (기술 개발, 혁신 프로젝트, 특허)
        5. 지속가능경영 (ESG 성과, 사회적 책임)
        6. 재무 성과 (연간 실적, 수익성, 재무 건전성)
        7. 주주가치 창출 (배당, 주가 성과, 주주환원)
        8. 내년도 전략 과제 (다음 해 주요 과제와 전략)
