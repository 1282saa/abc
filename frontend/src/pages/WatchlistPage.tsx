...
const CompanyCard: React.FC<{
  ...
  >
    ...
    <p className="text-sm text-gray-600 dark:text-gray-400">{company.category}</p>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="mt-3 text-xs text-gray-500">
              {articles[currentIndex].provider} · {articles[currentIndex].category}
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
...
}> = ({ isOpen, onClose, selectedArticles, onGenerateSummary, summaryResult, isGenerating }) => {
        ...
        >
          ...
          {!summaryResult && !isGenerating && (
            <>
              <p className="mb-4 text-gray-600 dark:text-gray-400">
                {selectedArticles.length}개의 기사를 선택하셨습니다. 원하시는 요약 방식을 선택해주세요.
              </p>
          ...
          {isGenerating && (
            <div className="text-center py-12">
              <LoadingSpinner />
              <p className="mt-4 text-gray-600 dark:text-gray-400">AI가 요약을 생성하고 있습니다...</p>
            </div>
          )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {summaryResult.key_data.map((data, index) => (
                      <div key={index} className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-primary-600 dark:text-primary-400">{data.value}</p>
...
const WatchlistPage: React.FC = () => {
  ...
  const [selectedArticles, setSelectedArticles] = useState<Map<string, SelectedArticle>>(new Map());
          ...
          articles: [
            {
              ...
              summary: "새로운 사업 영역으로 확장하며 성장 동력 확보에 나섰다...",
              provider: "서울경제",
              url: "#",
              category: "기업",
              byline: "홍길동 기자",
    ...
    >
      ...
      {selectedCompany && (
        <>
          ...
          {/* AI 요약 섹션 */}
          <motion.div variants={itemVariants} className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-4">AI 요약 분석</h2>
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              최근 {selectedCompany.name} 관련 뉴스를 AI가 분석하여 핵심 내용을 요약해드립니다.
            </p>
